...
//}
//
//void thread_function3(int n,int a,int b)
//{
//	std::thread::id this_id = std::this_thread::get_id();			//获取线程ID
//
//	for (int i = 0; i < 5; i++) {
...
//
//	thread mythread33(thread_function3, 1,2,3);      // 传递初始函数作为线程的参数
//	if (mythread33.joinable())                  //判断是否可以成功使用join()或者detach()，返回true则可以，返回false则不可以
//		mythread33.join();                     // 使用join()函数阻塞主线程直至子线程执行完毕
//
